// Package signingkeys handles the admin signing-key API: list publishable keys,
// rotate the active key, and emergency-retire a retiring key.
package signingkeys
...
func NewHandler(pool *pgxpool.Pool, auditStore audit.Logger, encryptionKey []byte) *Handler {
	return &Handler{pool: pool, auditStore: auditStore, encryptionKey: encryptionKey}
}
...
func (h *Handler) Rotate(w http.ResponseWriter, r *http.Request) {
	...
	if err := RetireSweep(r.Context(), h.pool, h.auditStore, cutoff); err != nil {
...
func (h *Handler) GetList(w http.ResponseWriter, r *http.Request) {
	keys, err := NewStore().ListPublishable(r.Context(), h.pool)
	if err != nil {
...
func (h *Handler) EmergencyRetire(w http.ResponseWriter, r *http.Request) {
	actor, ok := users.CurrentUser(r.Context())
	if !ok {
		httputil.WriteError(w, http.StatusUnauthorized, "INVALID_SESSION", "Not authenticated.")
		return
	...
	if err != nil {
		httputil.WriteError(w, http.StatusBadRequest, "VALIDATION_ERROR", "Invalid kid.")
		return
	}
	...
	if errors.Is(err, pgx.ErrNoRows) {
		httputil.WriteError(w, http.StatusNotFound, "SIGNING_KEY_NOT_FOUND", "Signing key not found.")
		return
	}
	if err != nil {
	...
	switch status {
	case "active":
		httputil.WriteError(w, http.StatusConflict, "CANNOT_RETIRE_ACTIVE_KEY", "Cannot retire the active signing key. Rotate first, then retire the demoted key.")
		return
	case "retired":
		httputil.WriteError(w, http.StatusConflict, "ALREADY_RETIRED", "Signing key is already retired.")
		return
	case "retiring":
	default:
