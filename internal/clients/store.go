// Package clients owns the clients table: the Store type and the Client struct.
// Handles OIDC client CRUD, secret rotation with overlap window, and
// redirect-URI validation. Caller owns the connection.
package clients
...
func (s *Store) GetByID(ctx context.Context, q database.Querier, id string) (*Client, error) {
...
func (s *Store) GetByIDAny(ctx context.Context, q database.Querier, id string) (*Client, error) {
	uid, err := uuid.Parse(id)
	if err != nil {
		return nil, ErrClientNotFound
	}
...
func (s *Store) GetByIDForUpdate(ctx context.Context, q database.Querier, id string) (*Client, error) {
	uid, err := uuid.Parse(id)
	if err != nil {
		return nil, ErrClientNotFound
	}
...
func (s *Store) VerifySecret(ctx context.Context, q database.Querier, id, secret string) (bool, error) {
...
func (s *Store) List(ctx context.Context, q database.Querier, status string) ([]*Client, error) {
	var rows pgx.Rows
	var err error
...
// wildcards.
func (s *Store) ValidateRedirectURI(c *Client, presented string) bool {
...
func (s *Store) Create(ctx context.Context, q database.Querier, p CreateClientParams) (*Client, error) {
	row := q.QueryRow(ctx, `
		INSERT INTO clients (
...
func (s *Store) UpdateFields(ctx context.Context, q database.Querier, id string, p UpdateClientPatch) (*Client, error) {
	uid, err := uuid.Parse(id)
	if err != nil {
		return nil, ErrClientNotFound
	}
...
func (s *Store) RotateSecret(ctx context.Context, q database.Querier, id, newHash string, overlapTTL time.Duration) (*Client, error) {
	uid, err := uuid.Parse(id)
	if err != nil {
		return nil, ErrClientNotFound
	}
...
func (s *Store) Disable(ctx context.Context, q database.Querier, id string) error {
	uid, err := uuid.Parse(id)
	if err != nil {
...
func (s *Store) Enable(ctx context.Context, q database.Querier, id string) error {
	uid, err := uuid.Parse(id)
	if err != nil {
		return ErrClientNotFound
	}
