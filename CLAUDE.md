...
**Password reset flow (Sprint 6b M6).** `POST /api/password-reset/request` is enumeration-safe — always returns 200 with empty body regardless of whether the email matches a user. Timing parity via a dummy-Argon2id verify against a pre-computed hash in `NewPasswordResetHandler` (mirrors the login enumeration defense). Rate-limited 5/min per IP via `SCHLASS_PASSWORD_RESET_RATE_LIMIT`. Tokens are 32-byte `crypto/rand`, base64url-encoded; only an HMAC-SHA256 hash (peppered, see below) is stored in `password_reset_tokens` (migration 000019, peppering added in 000020). Email is dispatched fire-and-forget in a post-commit goroutine — no retry queue in v1. `POST /api/password-reset/confirm` validates the token under `SELECT ... FOR UPDATE` (covers not-found / expired / already-used as `INVALID_TOKEN` uniformly), validates the password against the current policy, Argon2id-hashes, updates `users.password_hash`, marks `used_at = now()`, audits `password_reset.completed` — then post-commit bumps `revoke_before` and wipes Valkey sessions via `session.Store.DeleteAllForUser` (mirrors admin reset-password). No rate limit on confirm; token possession is the auth. Audit events: `password_reset.requested` (metadata `{email_matched: bool, token_id?: uuid, email_hash_prefix?: string}`) + `password_reset.completed` (metadata `{token_id: uuid}`).

**super_admin accounts cannot self-reset via email (Sprint 6b PR 3).** ENISA NIS2 (Jun-2025) mandates phishing-resistant MFA for privileged accounts; TOTP + email-delivered reset links are both phishable in real-time. `PostRequest` checks `user.Role == "super_admin"` AFTER the dummy-verify and AFTER `matched` is set, then routes through the unmatched path (no token mint, no email, 200 response). Enumeration timing preserved because the Role check runs post-dummy-Argon2id and performs only one best-effort audit write against the pool — symmetric with the unknown-email path. Separate audit event `password_reset.admin_blocked` (metadata `{reason: "super_admin_cannot_self_reset"}`) distinguishes the two forensically.

**Recovery path for super_admin accounts.** Another super_admin can reset via `POST /api/users/:id/reset-password` (everyday path). When no other super_admin exists, operator CLI subcommand `schlass recovery-reset --email <addr>` mints a 1h single-use token gated by `SCHLASS_RECOVERY_MODE=1` env var. Implementation in `internal/recovery/reset.go` (`Run` = CLI entry, `Execute` = testable core). Audit event `password_reset.recovery_issued` with `actor_email="system:recovery"` and metadata `{token_id, target_email, invoker: {hostname, os_user}}`. Operator runbook: `docs/operator/recovery.md`.

**Token hashes are HMAC-peppered (Sprint 6b PR 3 / M1).** `password_reset_tokens.token_hash` is `HMAC-SHA256(pepper, plaintext)`. Pepper is HKDF-derived from `SCHLASS_ENCRYPTION_KEY` with `info="password_reset_token_hmac_pepper"`, 32 bytes, deterministic — re-derived at every startup, no persistence. Defends against offline brute-force on a leaked DB dump. Pepper lives on the `PasswordResetTokenStore` struct; constructor panics if not 32 bytes. Migration 000020 force-expires all outstanding tokens on deploy because plaintext is unrecoverable to re-hash; users see existing links fail and re-request.

**Nightly sweeper (Sprint 6b PR 3 / M2).** `internal/scheduler.StartSweeper` runs a ticker goroutine bound to the main context; each tick deletes `password_reset_tokens` with `expires_at < now() - 30d` and `authorization_codes` with `expires_at < now() - 7d` inside one tx, then writes a single `password_reset.cleanup_swept` audit row with `{reset_rows_deleted, auth_code_rows_deleted}` counts. Interval default 24h, overridable via `SCHLASS_SWEEPER_INTERVAL_SECS` (0 disables for tests). `RunSweepOnce` is exported so integration tests drive a single pass synchronously.
